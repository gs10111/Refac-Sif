...
| `mutant_clear_ota_flag_before_ap` | `MUTANT_CLEAR_OTA_FLAG_BEFORE_AP` | the OTA flag is spent before the access point exists — production's order | `test_wifi_status` | por que a tentativa de Wi-Fi desistiu, em palavras: o mapeamento de `wl_status_t` para frase, testado fora do alvo |
| `test_ota` T50 (call sequence), T51, T52 | T54, T55, T56, T57 — none reach that function with the flag set |
